...
class TestErrorPaths:
    def test_bootstrap_survives_missing_llm_router(self, tmp_memory):
        ...
        with patch("llm_router.call_llm", side_effect=Exception("no API key")):
...
# ============================================================
# G. Outcome Feedback Tests
# ============================================================

class TestOutcomeFeedback:
    @pytest.fixture
    def tmp_sync(self, tmp_path):
        """Create a temp sync_tasks.json with test tasks."""
        log_dir = str(tmp_path / "logs")
        os.makedirs(log_dir)
        sync_file = os.path.join(log_dir, "sync_tasks.json")
        with patch("sync.SYNC_TASKS_FILE", sync_file), \
             patch("sync.LOG_DIR", log_dir), \
             patch("outcome_feedback.LOG_DIR", log_dir):
            yield sync_file, log_dir

    def _write_tasks(self, sync_file, tasks):
        with open(sync_file, "w") as f:
            json.dump(tasks, f)

    def test_extracts_lessons_from_success(self, tmp_sync):
        """Successful task with high validation score produces a lesson."""
        sync_file, log_dir = tmp_sync
        from outcome_feedback import _extract_execution_lessons
        task = {
            "id": "task_001", "title": "Build API", "status": "completed",
            "task_type": "build", "source_domain": "crypto",
            "result": {"validation_score": 8, "artifacts": ["api.py"]},
        }
        lessons = _extract_execution_lessons(task)
        assert len(lessons) >= 1
        sources = [l["source"] for l in lessons]
        assert "execution_success" in sources
        assert "execution_artifact" in sources

    def test_extracts_lessons_from_failure(self, tmp_sync):
        """Failed task produces a failure lesson."""
        from outcome_feedback import _extract_execution_lessons
        task = {
            "id": "task_002", "title": "Deploy service", "status": "failed",
            "task_type": "deploy", "source_domain": "infra",
            "result": {"error": "Connection timed out after 30s"},
        }
        lessons = _extract_execution_lessons(task)
        sources = [l["source"] for l in lessons]
        assert "execution_failure" in sources
        assert "execution_timeout" in sources

    def test_handles_string_result_gracefully(self, tmp_sync):
        """Tasks with a string result (not dict) don't crash."""
        sync_file, log_dir = tmp_sync
        self._write_tasks(sync_file, [{
            "id": "task_003", "status": "completed",
            "result": "just a string",
        }])
        from outcome_feedback import get_completed_tasks
        with patch("sync.SYNC_TASKS_FILE", sync_file):
            tasks = get_completed_tasks()
            assert len(tasks) == 0  # Filtered out because result is not a dict

    def test_handles_missing_status_gracefully(self, tmp_sync):
        """Tasks missing the 'status' key don't crash."""
        sync_file, log_dir = tmp_sync
        self._write_tasks(sync_file, [{"id": "task_004", "result": {"score": 7}}])
        from outcome_feedback import get_completed_tasks
        with patch("sync.SYNC_TASKS_FILE", sync_file):
            tasks = get_completed_tasks()
            assert len(tasks) == 0

    def test_marks_tasks_as_processed(self, tmp_sync):
        """After processing, tasks get _feedback_processed=True."""
        sync_file, log_dir = tmp_sync
        self._write_tasks(sync_file, [{
            "id": "task_005", "status": "completed", "source_domain": "crypto",
            "title": "Test task", "task_type": "build",
            "result": {"validation_score": 9},
        }])
        from outcome_feedback import process_pending_feedback
        with patch("sync.SYNC_TASKS_FILE", sync_file), \
             patch("outcome_feedback.LOG_DIR", log_dir), \
             patch("research_lessons.LESSONS_DIR", os.path.join(log_dir, "lessons")):
            result = process_pending_feedback()
            assert result["processed"] == 1
            assert result["lessons_total"] >= 1

            # Second call should find nothing to process
            result2 = process_pending_feedback()
            assert result2["processed"] == 0

    def test_no_double_processing(self, tmp_sync):
        """Tasks already marked _feedback_processed are skipped."""
        sync_file, log_dir = tmp_sync
        self._write_tasks(sync_file, [{
            "id": "task_006", "status": "completed", "source_domain": "test",
            "title": "Done task", "task_type": "build",
            "result": {"validation_score": 8},
            "_feedback_processed": True,
        }])
        from outcome_feedback import get_completed_tasks
        with patch("sync.SYNC_TASKS_FILE", sync_file):
            tasks = get_completed_tasks()
    ...
    def test_feedback_stats(self, tmp_sync):
        """get_feedback_stats returns correct counts."""
        sync_file, log_dir = tmp_sync
        self._write_tasks(sync_file, [
            {"id": "t1", "status": "completed", "result": {"score": 7}, "_feedback_processed": True},
            {"id": "t2", "status": "failed", "result": {"error": "x"}},
            {"id": "t3", "status": "pending", "result": None},
        ])
