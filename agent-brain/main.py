...
def main():
    ...
    parser.add_argument("--strategy-impact", action="store_true", help="Measure strategy evolution impact across domains")
    parser.add_argument("--health-pulse", action="store_true", help="Long-horizon degradation detection (question diversity, stagnation, drift)")
    parser.add_argument("--process-feedback", action="store_true", help="Process pending execution outcome feedback (Hands → Brain)")
    ...
    # Dry Run & Cost Management
    parser.add_argument("--dry-run", action="store_true", help="Simulate full pipeline with mock LLM (zero API cost)")
    parser.add_argument("--conservative", action="store_true", help="Use cheapest models everywhere, minimize token usage")
    parser.add_argument("--estimate", action="store_true", help="Estimate cost before running (use with --rounds)")
    ...
    # Allow offline commands (no API key needed) 
    _OFFLINE_COMMANDS = ("validate", "readiness", "review", "review_cycles", "dry_run",
                         "estimate", "strategy_impact", "health_pulse",
                         "calibration", "claim_stats", "process_feedback",
                         "budget", "prune", "prune_dry", "seed", "plan",
                         "recommend", "analytics")
    ...
    # Apply conservative mode — override models and loop settings for minimum cost
    if getattr(args, 'conservative', False):
        _cfg.MODELS.update(_cfg.CONSERVATIVE_MODELS)
        _cfg.MAX_RETRIES = _cfg.CONSERVATIVE_MAX_RETRIES
        _cfg.MAX_TOOL_ROUNDS = _cfg.CONSERVATIVE_MAX_TOOL_ROUNDS
        _cfg.MAX_SEARCHES = _cfg.CONSERVATIVE_MAX_SEARCHES
        _cfg.MAX_FETCHES = _cfg.CONSERVATIVE_MAX_FETCHES
        _cfg.CONSENSUS_ENABLED = _cfg.CONSERVATIVE_CONSENSUS
        _cfg.CRITIC_ENSEMBLE = _cfg.CONSERVATIVE_CRITIC_ENSEMBLE
        print("  [CONFIG] CONSERVATIVE mode — cheapest models, minimal tokens, single attempt")

    # Dry-run and cost estimation (before control command dispatch)
    if getattr(args, 'dry_run', False):
        from dry_run import run_dry_session
        rounds = args.rounds if args.rounds else 1
        run_dry_session(args.domain, rounds=rounds)
        return
    if getattr(args, 'estimate', False):
        from dry_run import estimate_cycle_cost, display_cost_estimate
        rounds = args.rounds if args.rounds else 1
        conservative = getattr(args, 'conservative', False)
        est = estimate_cycle_cost(args.domain, rounds=rounds, conservative=conservative)
        display_cost_estimate(est)
        return
    ...
    if getattr(args, 'readiness', False):
        ...
        display_readiness()
        return
    if getattr(args, 'strategy_impact', False):
        from strategy_impact import run_impact_check, display_impact
        domain_filter = args.domain if args.domain != DEFAULT_DOMAIN else None
        results = run_impact_check(domain=domain_filter)
        display_impact(results)
        return
    if getattr(args, 'health_pulse', False):
        from degradation_detector import run_degradation_pulse, display_degradation_pulse
        domain_filter = args.domain if args.domain != DEFAULT_DOMAIN else None
        results = run_degradation_pulse(domain=domain_filter)
        display_degradation_pulse(results)
        return
    if getattr(args, 'process_feedback', False):
        from outcome_feedback import process_pending_feedback, get_feedback_stats
        domain_filter = args.domain if args.domain != DEFAULT_DOMAIN else None
        ...
        print(f"  Already processed:   {stats['feedback_processed']}")
        print(f"  Pending feedback:    {stats['pending_feedback']}")
        if stats['pending_feedback'] > 0:
            result = process_pending_feedback(domain=domain_filter)
            print(f"\n  Processed:           {result['processed']} tasks")
            print(f"  Lessons extracted:   {result['lessons_total']}")
            print(f"  Domains touched:     {', '.join(result['domains']) or 'none'}")
            if result.get("details"):
                print(f"\n  Details:")
                for d in result["details"]:
                    status_icon = "+" if d["status"] == "completed" else "x"
                    print(f"    [{status_icon}] {d['task_id']}: {d['lessons_fed_back']} lesson(s)")
        else:
            print("  Nothing to process — all feedback is up to date.")
        print()
        return
