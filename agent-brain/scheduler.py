...
def run_daemon(
    ...
    try:
        while not _daemon_stop_event.is_set():
            ...
            try:
                ...
                for alloc in allocation:
                    ...
                    for r in range(rounds):
                        ...
                        try:
                            ...
                            # Daemon-side dedup: reject questions too similar to recent ones
                            if question:
                                try:
                                    from memory_store import is_duplicate_question
                                    dedup_attempts = 0
                                    while is_duplicate_question(domain, question) and dedup_attempts < 2:
                                        _log_daemon(f"  Dedup: skipping similar question, regenerating...")
                                        question = get_next_question(domain)
                                        dedup_attempts += 1
                                        if not question:
                                            break
                                except Exception:
                                    pass  # Dedup failure shouldn't block research
                ...
                if not require_approval:
                    ...
                    if hands_results:
                        ...
                        # === Outcome Feedback: feed execution results back to Brain ===
                        try:
                            from outcome_feedback import process_pending_feedback
                            fb = process_pending_feedback()
                            if fb["processed"] > 0:
                                _log_daemon(
                                    f"Outcome feedback: {fb['lessons_total']} lessons "
                                    f"from {fb['processed']} tasks → Brain"
                                )
                        except Exception as e:
                            _log_daemon(f"Outcome feedback error: {e}", "warning")
            ...
            # Degradation pulse + strategy impact (every 10 cycles)
            if cycle % 10 == 0:
                try:
                    from degradation_detector import run_degradation_pulse
                    pulse = run_degradation_pulse()
                    alerts = pulse.get("alerts", [])
                    if alerts:
                        high_alerts = [a for a in alerts if a["severity"] == "high"]
                        _log_daemon(f"Degradation pulse: {len(alerts)} alert(s) "
                                    f"({len(high_alerts)} high severity)")
                        for a in high_alerts[:3]:
                            _log_daemon(f"  [{a['type']}] {a['domain']}: {a['detail']}", "warning")
                except Exception as e:
                    _log_daemon(f"Degradation pulse error: {e}", "warning")

                try:
                    from strategy_impact import run_impact_check
                    impact = run_impact_check()
                    regressions = impact.get("regressions", [])
                    if regressions:
                        for r in regressions:
                            _log_daemon(f"Strategy regression: {r['domain']} {r['version']} "
                                        f"dropped {r['drop_pct']:.0%}", "warning")
                    stale = impact.get("stale_closed", 0)
                    if stale > 0:
                        _log_daemon(f"Closed {stale} stale evolution entries")
                except Exception as e:
                    _log_daemon(f"Strategy impact check error: {e}", "warning")
