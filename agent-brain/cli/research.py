...
def run_auto(domain: str, rounds: int = 1):
    ...
    for round_num in range(1, rounds + 1):
        ...
        if not question:
            stats = get_stats(domain)
            if stats["count"] == 0:
                # Cold domain — use bootstrap system (orientation + progressive questions)
                try:
                    from domain_bootstrap import is_cold, bootstrap_domain, get_bootstrap_question
                    if is_cold(domain):
                        print(f"[BOOTSTRAP] Cold domain '{domain}' — running bootstrap...")
                        bootstrap_domain(domain)
                    question = get_bootstrap_question(domain)
                    if question:
                        print(f"[BOOTSTRAP] Using bootstrap question for cold domain")
                except Exception:
                    question = None
                ...
                if not question:
                    ...
                    print(f"[SEED] Fallback to {curated} seed question")
            else:
