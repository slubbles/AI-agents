...
Focus: Make it possible to verify the system and run cycles with absolute minimum credit expenditure.

### 1. Dry-Run Mode (`dry_run.py`, `--dry-run`)
- Full pipeline simulation with synthetic LLM responses — zero API cost
- Traces: bootstrap check → strategy load → question gen → research → prescreen → critique → quality gate → memory store → task creation → calibration → run logging → stats
- Each step reports pass/fail with details
...
- Overrides ALL model assignments to CHEAPEST_MODEL (DeepSeek V3.2)
- Single attempt per question (no retry loop)
- 4 tool rounds (vs 8), 5 searches (vs 10), 3 fetches (vs 8)
- No consensus, no critic ensemble
- Estimated ~$0.01-0.02 per cycle vs ~$0.05-0.15 standard
- Activated via CLI flag, applies to any command (--auto, --daemon, etc.)
- Config constants: CONSERVATIVE_MODELS, CONSERVATIVE_MAX_RETRIES, etc.

### 3. Cost Estimator (`--estimate`)
- Shows expected cost before spending credits
- Breaks down by agent role with model and per-round cost
- Detects cold domains and adds bootstrap cost
- Compares standard vs conservative cost automatically
- Works offline (no API key needed)

### Files created
- `agent-brain/dry_run.py` — Pipeline simulation and cost estimation

### Files modified
- `agent-brain/config.py` — Conservative mode constants (models, limits, toggles)
- `agent-brain/main.py` — --dry-run, --conservative, --estimate CLI commands + activation logic
- `transistor-mindset.md` — Updated verification steps and budget section

---

## Session 36 — Guarantee #4 and #6 Enforcement

Focus: Close the two weakest guarantees in the transistor spec.

### Guarantee #4: Strategy Evolution Measurably Improves (`strategy_impact.py`)

**Problem**: After a strategy is confirmed, no post-approval validation exists. Stale evolutions sit as "pending" forever. No cumulative signal for "has evolution actually helped?"

**Implementation**:
- `get_strategy_impact(domain)` — compares avg scores across all strategy versions, computes trend (improving/declining/flat), total improvement delta
- `check_post_approval_regression(domain)` — tracks next 10 outputs after confirmation, flags if performance drops >15% from trial average
- `close_stale_evolutions(domain)` — closes pending evolution entries older than 14 days
- `run_impact_check()` — full analysis across all domains + regression detection + stale cleanup
- `--strategy-impact` CLI command (offline, no API cost)
- Integrated into daemon: runs every 10 cycles

### Guarantee #6: No Degradation During Long Runtime (`degradation_detector.py`)

**Problem**: Monitoring catches acute issues but slow decay over 50-100 cycles goes undetected.

**Implementation**:
- `check_question_diversity(domain)` — pairwise word overlap in recent questions; diversity score 0-1, verdict healthy/declining/repetitive
- `check_score_stagnation(domain)` — compares early vs recent window averages; detects plateau (flat + low variance)
- `check_strategy_drift(domain)` — word overlap between current strategy and v001; flags drift beyond 60% threshold
...
- `run_degradation_pulse()` — all checks across all domains, sorted alerts by severity
- `--health-pulse` CLI command (offline, no API cost)
- Integrated into daemon: runs every 10 cycles

### Daemon Question Dedup (scheduler.py)

**Problem**: Question dedup only ran in CLI mode (`cli/research.py`). The daemon generated questions without checking similarity to recent ones, wasting cycles on repetitive research.

**Fix**: Added `is_duplicate_question()` check in the daemon research loop with 2 regeneration attempts before accepting. Uses existing RAG/TF-IDF dedup from `memory_store`.

### Files created
- `agent-brain/strategy_impact.py` — Post-approval validation, regression detection, stale cleanup
- `agent-brain/degradation_detector.py` — Long-horizon degradation detection (5 check types)

### Files modified
- `agent-brain/scheduler.py` — Daemon question dedup + degradation pulse + strategy impact integration
- `agent-brain/main.py` — --strategy-impact, --health-pulse CLI commands
- `transistor-mindset.md` — Added guarantee enforcement section for #4 and #6

---

## Session 37 — Outcome Feedback Loop + Architectural Audit

**Goal**: Close the last structural gap in the core loop (Outcome Feedback), fix discovered bugs, and produce a confidence score against transistor-mindset.md.

### Outcome Feedback (`outcome_feedback.py`)

**Problem**: The core loop explicitly includes "→ Outcome Feedback → Repeat" but had zero implementation. Brain created tasks, Hands executed them, results were stored — but nothing fed execution results BACK to Brain. The loop was open.

**Implementation**:
- `get_completed_tasks()` — finds tasks with results that haven't been feedback-processed
- `_extract_execution_lessons()` — extracts structured lessons from success/failure
  - Success: "approach validated in practice" → research lesson with source `execution_success`
  - Failure: "insight may not be directly actionable" → lesson with source `execution_failure`
  - Pattern detection: timeout → "break into smaller steps"; access denied → "account for constraints"
- `process_task_feedback()` — feeds lessons into `research_lessons.add_lesson()`
- `process_pending_feedback()` — batch processor, marks tasks `_feedback_processed`
- `get_feedback_stats()` — overview of processed vs pending feedback
- Logs all events to `outcome_feedback.jsonl`
- Zero API cost — pure data extraction

### Bug Fixes

1. **Daemon dedup argument order**: `is_duplicate_question(question, domain)` → `is_duplicate_question(domain, question)` — arguments were reversed, matching nothing
2. **`run_auto` bypassed bootstrap**: Cold domains in `--auto` mode used `get_seed_question()` instead of the full bootstrap system. Now runs `bootstrap_domain()` + `get_bootstrap_question()` with seed fallback
3. **Missing offline commands**: `--calibration`, `--claim-stats`, `--process-feedback` added to `_OFFLINE_COMMANDS` (now work without API key)

### Wiring
- Daemon (`scheduler.py`): Outcome feedback runs automatically after Hands task execution
- CLI (`main.py`): `--process-feedback` command for manual review
- `cli/research.py`: `run_auto` now uses full bootstrap for cold domains

### Files created
- `agent-brain/outcome_feedback.py` — Execution outcome → Brain learning

### Files modified
- `agent-brain/scheduler.py` — Outcome feedback after Hands execution + daemon dedup arg fix
- `agent-brain/main.py` — --process-feedback CLI + offline commands fix
- `agent-brain/cli/research.py` — Bootstrap integration for run_auto cold domains
- `transistor-mindset.md` — Documented Outcome Feedback + --process-feedback CLI

---

## Session 38 — Deep Audit + Bug Hardening

**Goal**: Systematic audit of every module against the transistor-mindset.md spec. Find and fix real bugs, not just missing features.

### Bugs Fixed (13 total)

**HIGH severity (crash paths):**
1. `domain_bootstrap.py` — Unguarded import in `try_auto_transfer()` could crash entire bootstrap if `agents.cross_domain` had import errors. Wrapped all imports + calls in single try/except.
2. `outcome_feedback.py` — `KeyError` on malformed task entries missing `"status"` key. Changed to `.get()`.
3. `outcome_feedback.py` — `AttributeError` when task result was a string instead of dict. Added `isinstance(result, dict)` guard.

**MEDIUM severity (silent bugs):**
4. `domain_bootstrap.py` — No idempotency for in-progress bootstraps. Added guard to skip re-running when orientation already exists.
5. `outcome_feedback.py` — TOCTOU race on `sync_tasks.json` (load → process → re-load → save could overwrite concurrent changes). Refactored to single load-process-mark-save.
6. `main.py` — `_OFFLINE_COMMANDS` missing 7 commands that don't need API key (`budget`, `prune`, `prune_dry`, `seed`, `plan`, `recommend`, `analytics`). Added all.
7. `dry_run.py` — Cost estimator ignored claim verifier, synthesizer, and meta-analyst costs. Added all three with periodic multipliers.
8. `degradation_detector.py` — Short-word filter (`len(w) > 3`) stripped important acronyms (AI, LLM, API, SQL, GPU). Lowered to `> 2`.

**LOW severity (wrong results, dead code):**
9. `domain_calibration.py` — Median calculation biased upward for even-count lists. Fixed with proper even/odd handling.
10. `memory_lifecycle.py` — Dead `stale_count` variable from incomplete refactor. Removed.
11. `memory_lifecycle.py` — `total_stale = ...` used assignment instead of accumulation. Fixed to `+=`.
12. `strategy_impact.py` — Timestamp sorting with mixed empty values could misorder strategy versions. Filtered empty timestamps before `min()`.
13. `degradation_detector.py` — Tokenizer change from `> 3` to `> 2` preserves 3-char technical terms.

### Test Coverage Extended
- 7 new tests for `outcome_feedback.py` in `test_transistor.py`:
  - Success lesson extraction, failure lesson extraction, string result handling, missing status handling, task marking, no-double-processing, feedback stats
- Dry-run pipeline extended from 12 to 15 steps:
  - Added: strategy evolution check, outcome feedback check, degradation pulse check

### Files modified
- `agent-brain/domain_bootstrap.py` — Try/except + idempotency guard
- `agent-brain/outcome_feedback.py` — KeyError/AttributeError/TOCTOU fixes
- `agent-brain/main.py` — 7 missing offline commands added
- `agent-brain/dry_run.py` — 3 missing cost roles + 3 missing pipeline steps
- `agent-brain/domain_calibration.py` — Correct median calculation
- `agent-brain/memory_lifecycle.py` — Dead code removal + accumulation fix
- `agent-brain/strategy_impact.py` — Empty timestamp guard
- `agent-brain/degradation_detector.py` — Improved tokenizer threshold
- `agent-brain/tests/test_transistor.py` — 7 new outcome feedback tests
